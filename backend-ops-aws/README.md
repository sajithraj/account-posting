...
| `GET` | `/v2/payment/account-posting/{id}` | Fetch posting with all legs |
...
`POST /v2/payment/account-posting` (posting creation) is **not** handled here — it belongs to `backend-aws` and returns **404** if called against this Lambda.
...
- **Retry locking** — `acquireRetryLock` performs a DynamoDB conditional write setting `retryLockedUntil`. If the lock is already held, that posting is skipped (`skippedLocked++`) to prevent duplicate concurrent retries.
- **Retry flow** — each candidate posting re-publishes its `requestPayload` to SQS with `requestMode=RETRY`. `backend-aws` SQS consumer picks it up for processing.
- **Manual leg override** — `PATCH .../transaction/{order}` sets `mode=MANUAL`, updates `status`, `reason`, and `updatedBy`. `attemptNumber` is not incremented.
- **Config CRUD** — routing configs control which external systems (CBS / GL / OBPM) receive legs and in which order. Changes take effect on the next create or retry.
