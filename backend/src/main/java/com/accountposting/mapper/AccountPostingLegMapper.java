...
public interface AccountPostingLegMapper {
    ...
    AccountPostingLegRequest toCreateLegRequest(
    ...
    /**
     * Converts an external call result to an {@link UpdateLegRequest}; sets {@code postedTime} to now.
     */
    @Mapping(target = "postedTime", expression = "java(java.time.Instant.now())")
    UpdateLegRequest toUpdateLegRequest(ExternalCallResult result);
    ...
    default LegMode resolveMode(AccountPostingLegRequest request) {
        return request.getMode() != null ? request.getMode() : LegMode.NORM;
    }

    default String resolveOperation(AccountPostingLegRequest request) {
