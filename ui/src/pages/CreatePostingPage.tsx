    ...
    <div style={{ maxWidth: 600 }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <FormField label="Source Name *">
          <input required value={form.sourceName} onChange={set('sourceName')} placeholder="e.g. ONLINE_BANKING" />
        </FormField>
        <FormField label="Request Type *">
          <input required value={form.requestType} onChange={set('requestType')} placeholder="e.g. PAYMENT" />
        </FormField>
        <FormField label="Amount *">
          <input required type="number" step="0.0001" min="0.0001" value={form.amount} onChange={set('amount')} />
        </FormField>
        <FormField label="Currency *">
          <input required value={form.currency} onChange={set('currency')} maxLength={3} placeholder="USD" />
        </FormField>
        <FormField label="Credit / Debit *">
          <select value={form.creditDebitIndicator} onChange={e => setForm(f => ({ ...f, creditDebitIndicator: e.target.value as CreditDebitIndicator }))}>
        ...
        <FormField label="Requested Execution Date *">
          <input required type="date" value={form.requestedExecutionDate} onChange={set('requestedExecutionDate')} />
        </FormField>
        ...
        <button type="submit" disabled={mutation.isPending}
          style={{ padding: '10px 20px', backgroundColor: '#2980b9', color: 'white', border: 'none', borderRadius: 4, cursor: 'pointer' }}>
